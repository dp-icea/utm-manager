...
class FlightStripMongoDBAdapter(FlightStripRepositoryPort):
    ...
    async def create(self, flight_strip: FlightStrip) -> FlightStrip:
        ...
        try:
            ...
            existing = await self.collection.find_one({"id": flight_strip.id})
            if existing:
                raise ValueError(f"Flight strip with ID '{flight_strip.id}' already exists")
    ...
    async def get_by_id(self, flight_strip_id: str) -> Optional[FlightStrip]:
        ...
        try:
            doc = await self.collection.find_one({"_id": ObjectId(flight_strip_id)})
            return self._from_document(doc) if doc else None
        except Exception as e:
            logging.error(f"Error retrieving flight strip by ID {flight_strip_id}: {e}")
            return None
    
    async def get_by_flight_id(self, flight_id: str) -> Optional[FlightStrip]:
        """Retrieve flight strip by flight ID"""
        try:
            doc = await self.collection.find_one({"id": flight_id})
            return self._from_document(doc) if doc else None
        except Exception as e:
            logging.error(f"Error retrieving flight strip by flight ID {flight_id}: {e}")
            return None
    ...
    async def update(self, flight_strip: FlightStrip) -> FlightStrip:
        ...
        try:
            ...
            result = await self.collection.replace_one(
                {"_id": ObjectId(flight_strip.id)}, 
                doc
            )
            
            if result.matched_count == 0:
                raise ValueError(f"Flight strip with ID {flight_strip.id} not found")
            
            return flight_strip
            
        except Exception as e:
            logging.error(f"Error updating flight strip {flight_strip.id}: {e}")
            raise
    
    async def delete(self, flight_strip_id: str) -> bool:
        """Delete flight strip by ID"""
        try:
            result = await self.collection.delete_one({"_id": ObjectId(flight_strip_id)})
            return result.deleted_count > 0
        except Exception as e:
            logging.error(f"Error deleting flight strip {flight_strip_id}: {e}")
            return False
    ...
    async def list_by_flight_area(self, flight_area: FlightArea) -> List[FlightStrip]:
        """Get flight strips by flight area"""
        try:
            cursor = self.collection.find({"flight_area": flight_area.value}).sort("takeoff_time", 1)
            docs = await cursor.to_list(length=None)
            return [self._from_document(doc) for doc in docs]
        except Exception as e:
            logging.error(f"Error listing flight strips by flight area {flight_area}: {e}")
            return []
    ...
    async def exists(self, flight_strip_id: str) -> bool:
        ...
        try:
            count = await self.collection.count_documents({"_id": ObjectId(flight_strip_id)})
            return count > 0
        except Exception as e:
            logging.error(f"Error checking flight strip existence {flight_strip_id}: {e}")
            return False