...
class EventMonitor:
    ...
    def log_event(self, event_stream: str, correlation_id: str, method: str, 
                  path: str, success: bool, error_message: str = ""):
    ...
    def get_events_by_stream(self, event_stream: str) -> List[EventLog]:
        ...
        return [log for log in self.event_logs if log.event_stream == event_stream]
    ...
    def print_summary(self):
        ...
        if self.stats['total_events'] > 0:
            success_rate = (self.stats['successful_events'] / self.stats['total_events']) * 100
            print(f"Success Rate: {success_rate:.1f}%")
        
        print("\n=== Recent Events ===")
        for log in self.get_recent_events(5):
            status = "✓" if log.success else "✗"
            print(f"{status} {log.timestamp.strftime('%H:%M:%S')} - {log.event_stream} "
